...
bool DatasetWriter::addLidarMeasurement(const Time& /*stamp*/, const Eigen::Vector3d& /*rayMeasurement*/) {
    LOG(ERROR) << "No addLidar Callback implemented for DatasetWriter.";
    return false;
}

bool DatasetWriter::addDepthMeasurement(const okvis::Time& /*stamp*/,
                                       const cv::Mat& /*depthImage*/,
                                       const std::optional<cv::Mat>& /*sigmaImage*/) {
    LOG(ERROR) << "No addDepth Callback implemented for DatasetWriter.";
    return false;
}
